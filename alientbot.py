...
class AlienBot:
  ...
  def __init__(self):
    self.alienbabble = {'describe_planet_intent': r'.*(tell|what).*(me|is).*(about|your).*planet',
                        'answer_why_intent': r'.*why.*are.*you.*',
                        'cubed_intent': r'.*can.*you.*cube.* (\d+)'
                            }
  ...
  # Define .chat() next:
  def chat(self):
    reply = input(random.choice(self.random_questions) + "/n >> ").lower()
  ...
  def cubed_intent(self, number):
    cubed = str(int(number)*int(number)*int(number))
    return f"The cube of number {number} is {cubed}. Isn't that cool? "
  # Define .no_match_intent():
  def no_match_intent(self):
    responses = ("Please tell me more ", "Tell me more! ", "Why do you say that? ", "I see, How do think ", "How do you think I feel when you say that? ")
    return random.choice(responses)
